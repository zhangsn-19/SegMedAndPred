...
if __name__ == "__main__":
    ...
    # 实例化数据集对象
    dataset = ScarDataset(root_dir='final_labeled/', transform=train_transforms)
    ...
    images = []
    for i in range(1, 5):
        images.append(torch.tensor(Image.open("test/" + str(i) + ".jpg")))
    ...
    for cnt, image in enumerate(images):
        ...
        plt.title('Image')
        plt.subplot(1, 3, 2)
        plt.imshow(boundary_mask_pred.cpu().squeeze(), cmap='gray')
        plt.title('Predicted BODY BOUNDAY Mask')
        # plt.subplot(1, 3, 2)
